class Api::V1::UsersController < ApplicationController
  ...
  def facebook
    if params[:facebook_access_token]
      ...
      #"Facebook no longer provides access to email and public profiles for individual verification."
      user = User.find_by email: 'seth@gmail.com'
      if user
        ...
        if user.save
          json_response "Facebook Login Successfully", true, {user: user}, :ok
        else
            json_response user.errors, false, {}, :unprocessable_entity
        end
      end
    else
      json_response "Invalid Facebook Token", false, {}, :forbidden
    end
  end
end